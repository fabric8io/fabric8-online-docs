...
. Select *Scenario Driven Planning* from the drop-down menu for templates. This templates creates Spaces based on Scenarios, which are real-world problems from the user's point of view. The Scenarios deliver Value Propositions and are realized based on (user) Experiences.
